...
if sys.platform == "win32":
    ...
    from macos.scanner.fs_walker import (
        EntryKind,
        walk_filesystem,
        walk_specified_dirs,
        get_priority_scan_dirs,
    ...
    BINARY_EXTENSIONS: frozenset = frozenset()   # macOS detects by Mach-O header magic, not extension
    SKIP_PATH_PREFIXES: frozenset = frozenset()

    def walk_drives():
        """macOS stub: use walk_filesystem() instead."""
        return walk_filesystem()

    def walk_dir_for_manifests(path: str):
        """macOS stub: walk_specified_dirs handles this."""
        yield from walk_specified_dirs([path])

    def get_local_fixed_drives():
        """macOS stub: returns root mount point."""
        return ["/"]
    ...
    # Linux (and any future non-Windows, non-macOS platform)
